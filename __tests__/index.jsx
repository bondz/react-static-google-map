...
describe("Components", () => {
  ...
  test("It throws if size property is not set", () => {
    console.error = vi.fn();
    expect(() => render(<StaticGoogleMap />)).toThrow();
  ...
  test("It renders composite child elements", () => {
    const RedMarker = ({ color, ...props }) => <Marker color="red" {...props} />;
  ...
  test("Direction renders path", async () => {
    const testStrategy = vi.fn(() => Promise.resolve("test"));
  ...
  test("A span is rendered if image generation failed", async () => {
    expect.assertions(4);
    const testStrategy = vi.fn(() => Promise.reject("test"));
    console.error = vi.fn();
